...
Also good practice to think about units, note that $V_\mathrm{max}$ and $V^{app}_\mathrm{max}$ have the same units ($\mathrm{M\,s^{-1}}$), so their ratio is dimensionless:

$$
\frac{V_\mathrm{max}}{V^{app}_\mathrm{max}}
\quad\text{units:}\quad
\frac{\mathrm{M\,s^{-1}}}{\mathrm{M\,s^{-1}}} = 1.
$$

Therefore the units of $K_I$ are

$$
K_I \quad\text{units:}\quad
\frac{\mathrm{M}}{(1) - 1}
